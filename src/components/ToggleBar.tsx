...
export default function ToggleBar({pageMode, onToggle, renderedPlaceLength, isAnyLoading}:ToggleProps) {
    ...
    return (
        <div style={{zIndex: 99999}} className="fixed top-0 right-0 overflow-hidden h-8 bg-white text-xs sm:text-base">
            <div className="flex">
                ...
                {isAnyLoading !== null && <label htmlFor="pageMode" className="inline-flex float-right md:mr-5 md:ml-2 relative cursor-pointer">
                    ...
                    ></div>
                    <span className="ml--10 pl-4 sm:pl-0  font-medium text-gray-900">
                ...
                {isAnyLoading !== null && renderedPlaceLength == 0 && isToggled() ? <div>{`Click the map to find new stories with 🔍`}</div> : null}
                {isAnyLoading !== null && renderedPlaceLength == 0 && !isToggled() ? <div>{`<=========== Turn on FIND and click the map find new stories here`}</div> : null}
                {isAnyLoading !== null && renderedPlaceLength > 0 && <div className="items-end text-xs sm:text-base flex-shrink">
                        <div>
                        <div>[<Image className="inline-block" src={locIconFile.src} alt="new place" width={20} height={10}/> unimagined]</div>
                        <div>[<Image className="inline-block" src={openBookIconFile.src} alt="new place" width={20} height={10}/> existing]</div>
                        <div>[<Image className="inline-block" src={openBookRedIconFile.src} alt="new place" width={20} height={10}/> your finds]</div>
